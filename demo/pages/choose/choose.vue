<template>
	<view class="content">
		<button  style="width:100%;" @click="deuto">跳转到人脸检测</button>
		...
		methods: {
			...
			show(url){
				...
				get_list(){
					var that=this
					uni.request({
						url:"https://cn-cd-dx-6.natfrp.cloud:30496/get_llll/",
						data:{
						...
						success(res) {
							...
							for(var i in get_list){
								...
								"src":"https://cn-cd-dx-6.natfrp.cloud:30496/Uploadmp4/update_mzy1/"+get_list[i],
								})
								}
							}
		...
		check(){
		    ...
		    uni.chooseFile({
		        ...
		        success: (chooseImageRes) => {
		            ...
		            uni.uploadFile({
		                url: 'https://cn-cd-dx-6.natfrp.cloud:30496/cccc', 
		                ...
		                success(uploadFileRes) {
							...
							for(var i in eval(uploadFileRes.data)){
								...
								str+=eval(uploadFileRes.data)[i]+"\n"
							}
							console.log(str)
							if(str==""){
								that.text="在所有视频中未找到该物品"
							}
							else{
								that.text=str+"s";
							}
		...
		upload(){
		    ...
		    uni.chooseFile({
		        success: (chooseImageRes) => {
					...
					that.att="正在上传"
		            uni.uploadFile({
		                url: 'https://cn-cd-dx-6.natfrp.cloud:30496/get_ssss', 
...
.fontbest_match_index{width:200px; text-align:left; font-size:20px;}
</style>
